export const setPlaceholderText = () => {
    ...
    window.innerWidth < 400 ? (input.placeholder = 'City, State, Country') :
...
const updateWeatherLocationHeader = (message) => {
    ...
    if (message.indexOf('Lat:') !== -1 && message.indexOf('Long:') !== -1) {
        ...
        const lat = mapArray[0].indexOf('-') === -1 ? mapArray[0].slice(0, 10) :
            mapArray[0].slice(0, 11);

        const lon = mapArray[1].indexOf('-') === -1 ? mapArray[1].slice(0, 11) :
...
const fadeDisplay = () => {
    ...
    const sixThDay = document.getElementById('dailyForecast');
    sixThDay.classList.toggle('zero-vis');
    sixThDay.classList.toggle('fade-in');
}
